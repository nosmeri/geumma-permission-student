...
export default function StatusPage() {
  ...
  const renderPermitCard = (permit: Permit) => {
    ...
    return (
      <div
        key={permit.id}
        className="p-5 rounded-2xl border border-zinc-200 bg-white space-y-4 hover:border-zinc-350 transition-all duration-300"
      >
        ...
        <div className="space-y-2 text-xs">
          <div className="flex flex-col sm:flex-row gap-1 sm:gap-4">
            <span className="text-zinc-400 font-semibold w-16 shrink-0">신청 학생</span>
            <span className="text-zinc-700 break-all font-medium">{applicantsList}</span>
          </div>

          <div className="flex flex-col sm:flex-row gap-1 sm:gap-4">
            <span className="text-zinc-400 font-semibold w-16 shrink-0">이동 사유</span>
            <span className="text-zinc-700 font-medium">{permit.reason}</span>
          </div>

          {permit.status === "APPROVED" && permit.approver && (
            <div className="pt-2.5 mt-2.5 border-t border-zinc-100 flex flex-col sm:flex-row gap-1 sm:gap-4">
              <span className="text-zinc-400 font-semibold w-16 shrink-0">승인 교사</span>
    ...
    <div className="flex flex-col min-h-screen bg-zinc-50 text-zinc-900 font-sans antialiased selection:bg-zinc-900 selection:text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-zinc-200 bg-white/95 px-6 py-4 flex items-center justify-between">
        ...
        >
          ...
          <span className="text-xs font-bold text-zinc-650 hover:text-zinc-900 transition-colors">
            신청 페이지로
          </span>
        </div>
      ...
      <main className="flex-1 max-w-md w-full mx-auto px-5 py-8 z-10 space-y-6">
        ...
        <form onSubmit={handleSearch} className="flex gap-2">
          <input
            ...
            className="flex-1 h-11 px-4 rounded-xl border border-zinc-200 bg-white text-zinc-800 placeholder:text-zinc-400 focus:outline-none focus:border-zinc-900 transition-colors text-xs"
          />
          <button
            type="submit"
            className="h-11 px-4 rounded-xl bg-zinc-900 text-white hover:bg-black transition-colors text-xs font-semibold cursor-pointer shrink-0"
          >
            {searching ? "검색 중" : "검색"}
          </button>
        </form>
        ...
        <div className="flex items-center justify-between">
          ...
          {isSearchingState && (
            <button
              ...
              className="text-xs text-zinc-600 hover:text-zinc-900 font-bold transition-colors cursor-pointer"
            >
              내역 초기화
            </button>
          )}
        ...
        {loading ? (
          ...
          <div className="py-16 text-center border border-dashed border-zinc-200 rounded-2xl bg-white">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
            ...
            <p className="text-xs text-zinc-500 font-medium">신청 내역이 없습니다.</p>
            {!isSearchingState && (
              <button
                onClick={() => router.push("/")}
                className="text-xs text-zinc-800 font-bold mt-2 hover:underline cursor-pointer"
              >
                지금 허가원 신청하기
              </button>
            )}
